...
def generate_json(data_dir, filterNameBLE, filterNameMag, kalmanRBLE, kalmanRMag, kalmanQBLE, kalmanQMag, windowSizeBLE, windowSizeMag, noOfBeacons) :
	...
	save_path = os.getcwd() + "/geoMag-all-points-" + filterMag.get_name() + "/"
	...
	save_path = os.getcwd() + "/beacon-all-points-" + filterBLE[0].get_name() + "/"
	if not(os.path.isdir(save_path)) : 
		os.mkdir(save_path)
...
data_dir = os.getcwd() + "/final-readings/"
noOfBeacons = 10
filterNameBLE = "raw"
filterNameMag = "raw"
kalmanRBLE = 0.008
...
for filterMethod in ["raw", "kalman", "movingAverage"] :
	filterNameBLE = filterMethod
	filterNameMag = filterMethod
